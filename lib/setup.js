...
let generateUsers = function() {
  ...
  for (let i = 0; i < 100; i++) {
    let user = {};
    user.id = "u" + i;
    user.displayName = peopleNames.firstnames[Math.random() * peopleNames.firstnames.length] + peopleNames.lastnames[Math.random() * peopleNames.lastnames.length];
    user.friends = [];
    user.likedPages = [];
    users.push(user);
  }
...
let populateUsers = function() {
  ...
  for(let j = 0; j < Math.floor((Math.random() * 15) + 6); j++){
